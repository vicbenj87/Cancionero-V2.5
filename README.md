...
```
cancionero/
├── index.html          La app: índice, lectura y proyección
├── editor.html         El editor (se abre en local, no hace falta subirlo)
├── README.md           Esto
├── css/
│   ├── app.css         Estilos de las tres pantallas
│   ├── backgrounds.css Los 35 fondos animados
│   └── editor.css      Estilos del editor
└── js/
    ├── config.js       Ajustes (fondos, límites, comportamiento)
    ├── app.js          Lógica del índice, la lectura y el visor
    ├── editor.js       Lógica del editor
    └── data/
        └── canciones.js   ← EL ÚNICO ARCHIVO QUE SE SUSTITUYE
```

Diez archivos y ninguna dependencia: no hay que instalar ni compilar nada.
La carpeta `media/` solo hace falta si quieres fondos de foto o de vídeo, y se
explica más abajo cómo crearla.
...
>
> Aun así hay una red de seguridad: si `js/data/canciones.js` falta y la web está
> servida por http(s) —GitHub Pages, por ejemplo—, la app pide
> `js/data/canciones.json`, que es el otro formato que exporta el editor.
...
**Fondos CSS (35).** Están en `css/backgrounds.css`, uno por clase `.fondo--01` …
`.fondo--35`. No descargan nada y funcionan sin conexión. Para añadir el 36: escribe la
regla y sube `totalCss` a 36.

**Imágenes y vídeos.** No vienen en el paquete porque son tuyos. Crea las carpetas junto a
`index.html`, deja los archivos con estos nombres exactos y cambia `estrategia` a
`'imagen'`, `'video'` o `'mixto'`:
...
Para las fotos: JPG de 1920×1080, menos de 400 KB, sin texto y con el centro oscuro o
desenfocado, que es donde va la letra.
...
La lista sale **ordenada alfabéticamente** por título con criterio español: las tildes no
cuentan, la ñ se ordena junto a la n y `Salmo 2` va antes que `Salmo 10`. Los signos de
apertura no estorban: `¡Cantad al Señor!` cae en la C, no en un grupo aparte. El número de la izquierda sigue siendo el del
archivo de datos, ordene como ordene la lista: es el que usan los enlaces y el editor.
Para volver al orden del archivo de datos, pon `indice.ordenAlfabetico: false` en `js/config.js`.
...
muerta al repetir un coro. Se desactiva con `visor.bucle: false`.
...
encontrado. La duración se ajusta en `visor.parpadeoInicialMs` (0 lo desactiva); cada
latido dura 800 ms, que están en `@keyframes guiaFlecha` dentro de `css/app.css`, así que
...
## Traer las letras del cancionero web

El editor incluye un extractor para pasar el cancionero de Google Sites a este formato sin
teclear nada. **Se ejecuta en tu navegador, sobre tu propia página**: no hay servidor por
medio ni nada que salga de tu ordenador.

1. En el editor, pulsa **Traer de la web** y elige el TIPO de la página que vas a leer.
2. Copia el código y pégalo en la consola del navegador (`F12` → *Console*) teniendo abierta
   la página del cancionero. Se descarga un `.json` con todas sus canciones.
3. Vuelve al editor, pulsa **Importar canciones** y carga ese archivo.

Para juntar los dos cancioneros: importa primero el **Dominical** con *Reemplazar toda la
tabla*, y después el de **Santa Cena** con *Añadir a lo que ya hay*. Los títulos que estén en
las dos páginas pasan solos a `3 · Ambas` y conservan la letra que ya tenían.

### Qué hace el extractor con el texto

- Cada título de la web es un encabezado con ancla propia; de ahí saca dónde empieza y
  acaba cada canción.
- Las estrofas separadas por línea en blanco se convierten en diapositivas.
- Varias canciones de la web están escritas de corrido, con dobles espacios donde debería
  haber saltos de línea: los interpreta como saltos.
- Ningún bloque de más de ocho líneas se queda entero, porque proyectado no se lee: se parte
  en trozos parejos de unas cuatro líneas.
- Si salen más de 22 diapositivas, une el sobrante en la última.

Todo lo que haya tenido que decidir por su cuenta lo enumera en la consola, y esas canciones
son las que conviene repasar con el botón **Revisar** antes de exportar. La página tiene
además algunos títulos repetidos y varias canciones que aparecen dos y tres veces con nombres
distintos; **Revisar** también los señala.

### Un aviso que no es técnico

Las letras son de sus autores. Que tu iglesia las proyecte en el culto es un uso que se
cubre con una licencia de las habituales (CCLI y similares); publicarlas en una web abierta
es otra cosa. Si el repertorio va a un GitHub Pages público, merece la pena comprobar qué
cubre la licencia que tenéis.
