...
(function () {
  ...
  function abrirCancion(id, indiceDiapositiva) {
    ...
    var destino = dom.btnSiguiente.disabled ? dom.btnSalirVisor : dom.btnSiguiente;
  ...
  function conectarEventos() {
    ...
    dom.lista.addEventListener('click', function (ev) {
      ...
      // El botón de letra va primero: está dentro de la misma fila.
      var soloLetra = ev.target.closest('.ver-letra');
      if (soloLetra) {
        location.hash = '#/letra/' + soloLetra.dataset.letra;
        return;
      }
    ...
    dom.btnSalirVisor.addEventListener('click', irAlIndice);
