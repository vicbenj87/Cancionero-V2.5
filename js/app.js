...
(function () {
  ...
  function crearFilaCancion(c) {
    ...
    btn.setAttribute('aria-label',
      'Ver la letra completa de ' + c.titulo);

    /* Segundo botón: abrir la proyección. Sin él, la única puerta a las
       diapositivas pasaría por el botón grande de la fila, que ahora abre
       la letra (funciones invertidas a petición). */
    var letra = document.createElement('button');
    letra.type = 'button';
    letra.className = 'ver-letra';
    letra.dataset.letra = c.id;
    letra.title = 'Proyectar esta canción';
    letra.setAttribute('aria-label',
      'Proyectar ' + c.titulo + ', canción ' + c.id + ', ' + c.diapositivas.length +
  ...
  /* ======================================================================
     Lista semanal de acceso rápido
     ----------------------------------------------------------------------
     15 canciones fijas para el culto en curso, configuradas a mano en
     config.js (CFG.listaSemanal). Sin backend ni fechas del servidor: se
     compara con la fecha del propio navegador, a medianoche local.
     ====================================================================== */

  /** "AAAA-MM-DD" → Date a medianoche local, o null si el formato no cuadra.
   *  new Date('AAAA-MM-DD') se evita a propósito: ese formato lo interpreta
   *  como UTC y en América puede caer en el día anterior. */
  function fechaLocalDesdeTexto(txt) {
  ...
  /** De todas las entradas configuradas, la vigente hoy: la de `desde` más
   *  reciente que no sea posterior a la fecha actual. */
  function semanaVigente() {
    var cfg = CFG.listaSemanal;
    if (!cfg || !cfg.activo || !Array.isArray(cfg.semanas) || !cfg.semanas.length) {
      return null;
    }
    ...
    var elegida = null;
    cfg.semanas.forEach(function (semana) {
      var fecha = fechaLocalDesdeTexto(semana && semana.desde);
      if (!fecha || fecha > hoy) { return; }
      if (!elegida || fecha > elegida.fecha) { elegida = { fecha: fecha, ids: semana.ids }; }
    });
    return elegida;
  }

  /** Las canciones de la semana vigente, ya resueltas contra el repertorio.
   *  Los IDs que no existan (typo, canción borrada) se descartan en vez de
   *  romper la lista entera. */
  function cancionesDeLaSemana() {
    var vigente = semanaVigente();
    if (!vigente || !Array.isArray(vigente.ids)) { return []; }

    var lista = [];
    vigente.ids.slice(0, 15).forEach(function (id) {
      var c = buscarPorId(parseInt(id, 10));
      if (c) { lista.push(c); }
    });
    return lista;
  }

  function renderListaSemanal() {
    if (!dom.semanalLista) { return; }

    var canciones = cancionesDeLaSemana();
    var fragmento = document.createDocumentFragment();

    if (!canciones.length) {
      var vacio = document.createElement('li');
      vacio.className = 'semanal__vacio';
      vacio.textContent = 'Todavía no se ha definido la lista de esta semana.';
      ...
      canciones.forEach(function (c) {
        ...
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'semanal__item';
        btn.dataset.id = c.id;

        var num = document.createElement('span');
        num.className = 'semanal__numero';
        num.textContent = dosDigitos(c.id);

        var nombre = document.createElement('span');
        nombre.className = 'semanal__nombre';
        nombre.textContent = c.titulo;

        btn.appendChild(num);
        btn.appendChild(nombre);
        btn.setAttribute('aria-label', 'Ver la letra de ' + c.titulo);
    ...
    dom.semanalLista.innerHTML = '';
    dom.semanalLista.appendChild(fragmento);
  }

  function semanalAbierta() {
    return !!(dom.panelSemanal && !dom.panelSemanal.hidden);
  }

  function cerrarSemanal() {
    if (!semanalAbierta()) { return; }
    dom.panelSemanal.hidden = true;
    dom.btnSemanal.setAttribute('aria-expanded', 'false');
    document.removeEventListener('click', cerrarSemanalSiFuera, true);
    document.removeEventListener('keydown', cerrarSemanalConEsc);
  }

  function abrirSemanal() {
    if (semanalAbierta()) { return; }
    renderListaSemanal();
    dom.panelSemanal.hidden = false;
    dom.btnSemanal.setAttribute('aria-expanded', 'true');
    // En captura: así se cierra aunque el clic caiga en un elemento que
    // detiene la propagación normal (por ejemplo, dentro de un filtro).
    document.addEventListener('click', cerrarSemanalSiFuera, true);
    document.addEventListener('keydown', cerrarSemanalConEsc);
  }

  function cerrarSemanalSiFuera(ev) {
    if (dom.semanal.contains(ev.target)) { return; }
    cerrarSemanal();
  }

  function cerrarSemanalConEsc(ev) {
    if (ev.key === 'Escape') { cerrarSemanal(); }
  }
  ...
  function conectarEventos() {
    ...
    dom.lista.addEventListener('click', function (ev) {
      ...
      /* Funciones invertidas a petición: el botón "ver-letra" abre ahora la
         proyección y el botón "cancion" abre ahora la letra completa. Va
         primero porque está dentro de la misma fila. */
    ...
    /* --- Lista semanal de acceso rápido ---------------------------------- */
    if (dom.btnSemanal) {
      dom.btnSemanal.addEventListener('click', function (ev) {
        ev.stopPropagation();
        if (semanalAbierta()) { cerrarSemanal(); } else { abrirSemanal(); }
      });

      dom.panelSemanal.addEventListener('click', function (ev) {
        var item = ev.target.closest('.semanal__item');
        if (!item) { return; }
        cerrarSemanal();
        estado.regreso = '#/';
        location.hash = '#/letra/' + item.dataset.id;
      });
    }
  ...
  function iniciar() {
    dom = {
      ...
      semanal:       $('#semanal'),
      btnSemanal:    $('#btn-semanal'),
      panelSemanal:  $('#panel-semanal'),
      semanalLista:  $('#semanal-lista'),
    ...
    if (dom.semanal && CFG.listaSemanal && CFG.listaSemanal.activo === false) {
      dom.semanal.hidden = true;
    }
    ...
    obtenerDatos().then(function (datos) {
      ...
      cargarCanciones();
      renderIndice();
      renderListaSemanal();                    // ya con el repertorio cargado
