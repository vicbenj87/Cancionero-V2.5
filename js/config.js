...
window.CANCIONERO_CONFIG = {
  ...
  /* --- Lista semanal de acceso rápido ------------------------------------
     Botón "Esta semana" en el índice: 15 canciones fijas para el culto,
     sin tener que buscarlas. No hay servidor ni panel de administración
     (la app es estática), así que la lista se escribe aquí a mano, igual
     que se hace con el repertorio en el editor.

     Cada entrada de `semanas` fija los 15 IDs (el número de ficha del
     editor, el mismo que aparece a la izquierda del título en el índice)
     válidos desde la fecha `desde` (formato AAAA-MM-DD, pensada para un
     domingo) en adelante. La app usa siempre la entrada con `desde` más
     reciente que no sea posterior a hoy, así que basta con añadir una
     entrada nueva cada semana; las antiguas se pueden dejar tal cual, no
     estorban.
  ...
  listaSemanal: {
    ...
    semanas: [
      ...
      { desde: '22 de mayo del 2026',ids: [ID_Alaba, ID_Bendito, ID_CuanGrande, ID_Gozo, ID_EnJesus] }
    ]
  }
};
